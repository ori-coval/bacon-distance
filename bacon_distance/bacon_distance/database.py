...
class ActorMovie(Base):
    ...
    actor_id: Mapped[str] = mapped_column(
        String(10), ForeignKey("actors.nconst"), primary_key=True
    )
    movie_id: Mapped[str] = mapped_column(
        String(10), ForeignKey("movies.tconst"), primary_key=True
    )

    actor = relationship("Actor", back_populates="actor_movies")
    movie = relationship("Movie", back_populates="actor_movies")
    __table_args__ = (Index("idx_movie_actor", "movie_id", "actor_id"),)


class Actor(Base):
    __tablename__ = "actors"
    nconst: Mapped[str] = mapped_column(String(10), primary_key=True)
    primaryName: Mapped[str] = mapped_column(String(255), nullable=False, index=True)
...
class Movie(Base):
    ...
    tconst: Mapped[str] = mapped_column(String(10), primary_key=True)
